...
- **手写块**：工具栏「手写块」（Ctrl+Shift+K）打开画板，鼠标 / 手指 / 触控笔都能写；触控笔按压力、鼠标按速度变化线宽；画完存成工作区里的矢量 SVG（如 `assets/sketch-20260921_143012.svg`）并插入正文，点一下已插入的手写块即可再编辑（覆盖原文件，正文链接不变），导出 HTML / PDF 自动内联
