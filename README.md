...
- **双栏分屏**：两个文件并排编辑，点击窗格即聚焦，保存 / 查找 / 大纲 / 状态栏跟随聚焦窗格，各自独立自动保存
- **KaTeX 数学公式**：行内 `$...$`、块级 `$$...$$`
- **行内高亮**：选中文字点工具栏「高亮」或按 Ctrl+Shift+M 包成 `==高亮==`，渲染为底色标记，再点一次取消
- **代码块语法高亮**：按代码围栏语言着色（内置 highlight.js，192 种语言），五套主题各配一套代码配色
...
- **手写信纸模式**：可切换信纸横线、横线纸、方格纸与空白纸模板，内置手写体与京華老宋体，也支持导入中英文字体文件离线使用
- **导出**：单文件 HTML（样式与公式内联，离线可看）、PDF（系统打印）、复制为 HTML

**文件**

- **新建文档**：欢迎页一键开写或 Ctrl+N，无需先打开文件夹或已有文件；未命名文档首次保存（Ctrl+S）时弹「另存为」询问保存位置与文件名
- **跨文件替换**：全文搜索面板一键替换工作区全部匹配，替换前确认数量并自动备份本地历史快照；已打开文件智能同步
- **会话恢复**：重启自动回到上次的工作区与标签页，未保存内容与未命名文档作为草稿一并找回，应用意外退出也不丢
- **打开与另存为**：Ctrl+O 打开单个文件，Ctrl+Shift+S 另存为
- **拖拽打开**：把 .md / .txt 等文本文件或整个文件夹拖进窗口即可打开；安装版双击 .md / .txt 直接用本应用打开（重复启动自动转发给已有窗口）
- **文件树 + 多标签页**：打开本地文件夹，支持新建 / 重命名 / 删除，目录懒加载，标签页可拖拽排序；.txt 等纯文本也能直接编辑
...
- **Vim 模式**：纯文本编辑器支持完整 Vim 键位（设置中开关，状态栏显示 VIM 徽标）
...
- **Markdown 源码编辑模式（带 Vim）**：当前 Vim 仅覆盖纯文本编辑器，Vditor 富文本渲染器无法支持 Vim。计划为 Markdown 新增「CodeMirror 源码模式」编辑引擎（文件类型 × 编辑引擎二维切换），需同步打通查找替换、HTML 导出与三种预览模式的兼容
- 多光标编辑
- 导出 Word（.docx）
- 导出 Word / 长图
- 主题 CSS 自定义文件
